...
#'     will uses a system GATK from $PATH.
...
markDuplicates <- function(bamFile,
                           bamDir=NULL,
    ...
    output <- paste(gsub(pattern=".bam",
                    ...
                    sep=".")
    ### Settings of run.
    input <- paste("--INPUT", paste(path, bamFile, sep="/"), sep=" ")
    output <- paste("--OUTPUT", paste(path, output, sep="/"), sep=" ")
    metrics <- paste("--METRICS_FILE",
                     paste(path, paste(gsub(pattern=".bam",
                                            replacement="",
                                            x=bamFile),
                                       postfix,
                                       "metrics.txt",
    ...
    ### Settings for saving of standart errors.
    se <- ""
    if (SE == TRUE){
        se <- "STDERR.MARKDUP.TXT"
    }
    ### Running of command.
    system2(command=gatk_path,
            args=c("MarkDuplicates", input, output, metics, remove.d),
            stderr=se)
}
